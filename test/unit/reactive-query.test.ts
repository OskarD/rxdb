        ...
        new Array(isFastMode() ? 5 : 20)
            .fill(0).forEach((_v, runIdx) => {
                it('query.$ emits after insert with async storage (run ' + runIdx + ')', async () => {
                    ...
                    const resolveStalledQuery = pendingQueryResolve as unknown as ((result: any) => void);
                    pendingQueryResolve = null;
                    resolveStalledQuery(undefined);

                    // for subsequent queries, respond normally
                    // (we need to restore normal query behavior so re-queries work)
